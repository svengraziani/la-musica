...
Bounce jobs return a `graph_bounce` result manifest with range, format, normalization peaks,
`peakAfterDither`, and `explicitExport: true`.
...
`batch_project_mix_export`, and `stem_export` manifests with `explicitExport: true` and
`peakAfterDither` for each rendered artifact. Batch mix manifests include one item per requested
export. Stem manifests include one item per rendered track.
