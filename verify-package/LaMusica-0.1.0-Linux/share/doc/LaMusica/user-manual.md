...
language override, and diagnostics sharing. If no language override is set, LaMusica follows the
system display language and falls back to English when a locale is not bundled. Diagnostics sharing
is default-off and requires explicit granted consent; see
[Privacy And Diagnostics](release/privacy-and-diagnostics.md) for collected fields, endpoint
override, retention, and opt-out details. The Preferences window reflects the current session
preference state across Audio, MIDI, Plugins, MCP, Shortcuts, Language, and Privacy tabs. MCP
project mutation cannot be enabled unless MCP itself is enabled.
...
New Project opens the welcome/template flow. Empty creates a silent master-only project, Basic
Multitrack creates named audio tracks, Drum + Synth creates instrument tracks with MIDI and
automation, and Podcast / Voice creates host/guest tracks, a voice group, master routing, and
segment markers. Each template is authored through the same generic session commands used by CLI,
MCP, and menu actions, then saved as an independent `.Project.lamusica` bundle.
Legacy first-track starter commands remain available for compatibility with existing tutorials and
release fixtures. That starter creates generated drums, generated bass, a master track, section
markers, clips, routing, built-in starter devices, MIDI clip references, and automation lanes, so
old first-track projects can still be rendered without external assets.
The starter MIDI reference resolves to an eight-note C-minor bass phrase for the generated bass
track. Readiness reports include tempo, meter, section names, section positions, and track-role
counts, so smoke checks verify the legacy starter session is musically shaped, not just
structurally present. The bundled first-song tutorial project uses the same starter devices and
automation lanes for release-regression coverage.
...
- Inspect any project with structured output:
  `lamusica_cli query fixtures/empty.Project.lamusica --json`
- Preview and confirm a generic clip edit:
  `lamusica_cli edit MyProject.Project.lamusica set-clip-gain clip-1 -6 --dry-run --json`, then
  rerun with `--confirm <token>` from the preview.
- Render with explicit output controls:
  `lamusica_cli render MyProject.Project.lamusica mix.wav --format wav --bit-depth 24 --range 0:96000 --json`
- Print the project schema version: `lamusica_cli schema --json`
- Migrate an older project bundle in place: `lamusica_cli migrate OldProject.Project.lamusica --json`
