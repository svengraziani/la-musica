...
- Supply-chain checks recurse over every source `CMakeLists.txt` and `*.cmake` file outside generated build/package directories and reject unreviewed downloader integrations.
...
Run `cmake -DLAMUSICA_DEPENDENCY_LOCK_SELF_TEST=ON -P cmake/CheckDependencyLock.cmake`
to verify the lock rejects nested unreviewed downloader integrations such as `FetchContent`.
