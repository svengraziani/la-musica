...
## Realtime Deadline Gate

`tests/perf/rt_deadline_bench.cpp` is the blocking P26 realtime gate. It renders fixed 8, 16, 32,
and 64 track sessions block-by-block, records p99 block time, max block time, buffer deadline, xrun count, RSS, and measured WAV disk bytes, and appends a JSONL row to `tests/perf/rt-history.jsonl`.
Each row is keyed by `MachineContext` so changes can be compared against the runner that produced
them.

The gate fails on any xrun and fails when p99 block time is greater than or equal to the buffer period. History JSONL files are build artifacts, not source fixtures; update `tools/perf/README.md`
and release notes whenever thresholds, buffer size, or tracked scale points change.
